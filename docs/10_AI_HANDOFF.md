...
20. (v1.2.0부터) `app/settings.py`의 `save_settings()`는 반드시 `load_settings()`(지금
    저장된 값) 위에 덮어써야 한다 - DEFAULTS 위에만 덮어쓰면, 예를 들어 "이미지 도구 선택"만
    저장해도 "파일자동읽기 폴더지정" 설정이 기본값(꺼짐)으로 초기화돼버리는 버그가 있었다
    (반대 방향도 마찬가지). 두 설정 그룹은 서로 완전히 독립적으로 유지되어야 한다

21. (v1.2.0부터) `OrderEditor._confirm()`이 호출하는 `self.app._open_result_viewer()`는
    "이미지 도구 선택"에서 사용자가 프로그램을 지정했으면 그 프로그램으로, 지정하지 않았으면
    **기존 그대로** FastStone Image Viewer 자동 감지 → 없으면 탐색기 순으로 연다
    (`app/gui.py`의 `_open_result_viewer()`). 이 대체(fallback) 체인 자체는 절대 건드리지
    말 것 - "이미지 도구 선택" 기능은 이 체인의 맨 앞에 사용자 선택을 끼워 넣을 뿐, 기존
    동작을 대체하는 게 아니다

23. (v1.2.1부터) UI 섹션 번호는 "4. 파일자동읽기 폴더지정" → "5. 이미지 도구 선택" 순서다
...
24. (v1.2.1부터) "이미지 도구 선택"으로 고른 뷰어가 결과 폴더를 열었을 때 화면이 비어
    ...
    인자로 넘겨 실행해도, 그 프로그램이 시작 시 명령줄 인자를 읽어 폴더를 자동으로 여는
    로직이 없으면 그냥 빈 화면으로 뜬다. PhotoDedup 쪽에서 고칠 수 있는 부분이 아니므로,
    "이미지 도구 선택"에 등록하는 프로그램은 **실행 시 첫 번째 인자를 폴더 경로로 받아
    자동으로 그 폴더를 여는 기능이 있어야** 이 연동이 정상 동작한다. FastStone은 이미 이
    방식으로 동작해서 문제가 없었던 것이다. 자체 개발한 뷰어(FastImageAnnotator 등)를 여기
    등록하려면 그 프로그램의 시작 인자 처리 로직부터 확인/구현해야 한다

22. (v1.2.0부터) `app/viewers.py`의 `detect_viewers()`는 Windows 레지스트리(.jpg/.png
    확장자의 기본 연결 프로그램 + "다른 앱으로 열기" 후보)로 뷰어를 찾는데, 이 방식으로
    모든 프로그램을 찾을 수 있는 건 아니다(예: 설치 없이 폴더에 풀어서 쓰는 프로그램은
    레지스트리에 아무 흔적도 안 남는다). 그래서 GUI의 "이미지 도구 선택" 콤보박스에는
    항상 "찾아보기로 직접 선택..." 항목이 함께 있어야 한다 - 이걸 없애면 레지스트리에 안
    잡히는 프로그램은 선택할 방법이 아예 없어진다
```
...
| `app/viewers.py` | "이미지 도구 선택" - 레지스트리에서 이미지 뷰어 후보 프로그램을 찾아 목록으로 제공 |
